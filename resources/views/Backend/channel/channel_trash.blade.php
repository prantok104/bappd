                            ...
                            @forelse($channels_list as $key=>$channel)
                                <tr>
                                    ...
                                    <td class="text-right">
                                        ...
                                        <a href="{{ route('admin.channel.restore', lock($channel->id)) }}"
                                            title="Restore"><i class="las la-redo-alt text-info font-18"></i></a>
                                    </td>
                                </tr>
                            @empty
                                <td>
                                    <span class="badge badge-soft-danger p-3 d-block">Have no channel found in our system
                                        ):</span>
                                </td>
                            @endforelse
        ...
        $('body').on('click', '.deletebtn', function() {
            ...
            Swal.fire({
                ...
                if (result.value === true) {
                    ...
                    $.ajax({
                        url: "{{ URL::route('admin.channel.force.delete', '') }}/" + id,
                        type: "DELETE",
                        data: {
                            "_token": token,
                            "id": id
