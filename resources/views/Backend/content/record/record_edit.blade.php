...
@section('content')
    ...
    @php
        $record_genres = $record_categories = $record_genres = $record_casts = [];
    @endphp
    <!-- Page-Title -->
    <div class="row">
        <div class="col-sm-12">
            <div class="page-title-box">
                <div class="row">
                    <div class="col">
                        <h4 class="page-title">Record - Edit</h4>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Record edit</li>
                        </ol>
    ...
    <div class="col-lg-12">
        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-title d-flex align-items-center justify-content-between"> Edit

                    @if ($record->sequences_count > 0)
                    ...
                    @else
                        <a href="{{ route('admin.content.single.videos', lock($record->contents->id)) }}"
                            class="btn btn-primary"><i class="las la-angle-left"></i> Back</a>
                    @endif
            ...
            <!--end card-header-->
            <div class="card-body">
                <form method="POST" action="{{ route('admin.content.sequence.video.update', lock($record->id)) }}"
                    ...
                    <div class="form-row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="select2 mb-3 select2-multiple @error('categories') is-invalid @enderror"
                                ...
                                @foreach (optional($record)->categories as $category)
                                    @php
                                        $record_categories[] = $category->id;
                                    @endphp
                                @endforeach
                                @foreach ($categories as $key => $category)
                                    <option value="{{ $category->id }}"
                                        {{ in_array($category->id, $record_categories) ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="select2 mb-3 select2-multiple @error('genres') is-invalid @enderror"
                                ...
                                data-placeholder=" Choose">
                                @foreach (optional($record)->genres as $genre)
                                    @php
                                        $record_genres[] = $genre->id;
                                    @endphp
                                @endforeach

                                @foreach ($genres as $key => $value)
                                    <option value="{{ $value->id }}"
                                        {{ in_array($value->id, $record_genres) ? 'selected' : '' }}>
                                        {{ $value->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-control @error('channel') is-invalid @enderror" name="channel"
                                ...
                                @foreach ($channels as $key => $value)
                                    <option value="{{ $value->id }}"
                                        {{ $value->id == optional($record)->channels->id ? 'selected' : '' }}>
                                        {{ $value->name }}</option>
                                @endforeach
                            </select>
                            @error('channel')
                        ...
                        <div class="col-md-4 mb-3">
                            <label for="content">Content <span class="text-danger">( Optional )</span></label>
                            <select class="select2 mb-3 @error('content') is-invalid @enderror" name="content"
                                id="content" style="width: 100%" data-placeholder=" Choose">
                                <option value="">---Select the content---</option>
                                @foreach ($contents as $contenter)
                                    @if ($record->sequences_count > 0)
                                        <option value="{{ $contenter->id }}"
                                            {{ $contenter->id == optional($record->sequences)->contents->id ? 'selected' : '' }}>
                                            {{ $contenter->title }}
                                        </option>
                                    @else
                                        <option value="{{ $contenter->id }}"
                                            {{ $contenter->id == $record->content_id ? 'selected' : '' }}>
                                            {{ $contenter->title }}
                                        </option>
                                    @endif
                                @endforeach
                        ...
                        <div class="col-md-4 mb-3">
                            <label for="season">Season <span class="text-danger">( Optional )</span></label>
                            <select class="mb-3 @error('season') is-invalid @enderror form-control" name="season"
                                id="season" style="width: 100%" data-placeholder=" Choose">
                                <option value="">---Select the season---</option>
                                @if ($record->sequences_count > 0)
                                    @foreach ($record->sequences->contents->sequences as $key => $seaquence)
                                        <option value="{{ $seaquence->id }}"
                                            {{ $seaquence->id == optional($record)->sequences->id ? 'selected' : '' }}>
                                            {{ $seaquence->title }}
                                        </option>
                                    @endforeach
                                @endif
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" name="order" id="order" value="{{ $record->order }}"
                                class="form-control @error('order') is-invalid @enderror" readonly>
                            @error('order')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" name="release_date" id="mdate"
                                value="{{ $record->release_date }}" placeholder="YYYY-MM-DD"
                            ...
                            <select name="content_type" id="content_type" class="form-control">
                                @foreach ($content_types as $content_type)
                                    <option value="{{ $content_type->id }}"
                                        {{ $content_type->id == $record->source_id ? 'selected' : '' }}>
                                        {{ $content_type->content_types_name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('content_type')
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <select class="select2 mb-3 select2-multiple @error('casts') is-invalid @enderror"
                                ...
                                @foreach (optional($record)->casts as $cast)
                                    @php
                                        $record_casts[] = $cast->id;
                                    @endphp
                                @endforeach
                                @foreach ($casts as $key => $cast)
                                    <option value="{{ $cast->id }}"
                                        {{ in_array($cast->id, $record_casts) ? 'selected' : '' }}>
                                        {{ $cast->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="file" id="record_thumbnail" name="record_thumbnail"
                                ...
                                data-default-file="{{ $record->getFirstMediaUrl('record_thumbnail') }}" />
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="file" id="record_cover_photo" name="record_cover_photo"
                                class="dropify form-control"
                                data-default-file="{{ $record->getFirstMediaUrl('record_cover_photo') }}" />
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea id="video_description" class="editor-enable @error('video_description') is-invalid @enderror"
                                name="video_description">{{ $record->description }}</textarea>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select name="is_last" id="is_last"
                                ...
                                <option value="0" {{ $record->is_last == 0 ? 'selected' : '' }}>No</option>
                                <option value="1" {{ $record->is_last == 1 ? 'selected' : '' }}>Yes</option>
                            </select>
                            @error('is_last')
                                <div class="invalid-feedback">
                                    {{ $message }}
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select name="option" id="option"
                                ...
                                <option value="0" {{ $record->is_premium == 0 ? 'selected' : '' }}>Free</option>
                                <option value="1" {{ $record->is_premium == 1 ? 'selected' : '' }}>Premium</option>
                            </select>
                            @error('option')
                                <div class="invalid-feedback">
                                    {{ $message }}
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select name="downloadable" id="downloadable"
                                ...
                                <option value="0" {{ $record->is_downloadable == 0 ? 'selected' : '' }}>No</option>
                                <option value="1" {{ $record->is_downloadable == 1 ? 'selected' : '' }}>Yes</option>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="1" {{ $record->is_publish == 0 ? 'selected' : '' }}>Yes</option>
                                <option value="0" {{ $record->is_publish == 0 ? 'selected' : '' }}>No</option>
                    ...
                    <div class="form-group">
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="invalidCheck3">
                                Are you want to edit the episode ?
                            </label>
                            @error('ensure')
                                <div class="invalid-feedback">
                                    {{ $message }}
    ...
    <script>
        $(document).ready(function() {
            $('#content').on('change', function() {
                let contentId = $(this).find(":selected").val();
                $.ajax({
                    url: "{{ route('admin.content.get.season') }}",
                    method: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                        contentId: contentId
